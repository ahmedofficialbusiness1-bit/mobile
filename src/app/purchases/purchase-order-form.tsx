...
const TotalPriceDisplay = ({ control, index }: { control: any, index: number }) => {
    const quantity = useWatch({ control, name: `items.${index}.quantity` });
    const unitPrice = useWatch({ control, name: `items.${index}.unitPrice` });
    const total = (quantity || 0) * (unitPrice || 0);
    return <Input readOnly value={total.toLocaleString()} />;
};
                      ...
                      <div key={item.id} className="p-3 border rounded-md relative space-y-3">
                        <div className="flex justify-between items-start">
                           <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            />
                             ...
                             {fields.length > 1 && (
                                <Button type="button" variant="ghost" size="icon" className="text-destructive ml-2 mt-1" onClick={() => remove(index)}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                   <TotalPriceDisplay control={form.control} index={index} />
                                </FormControl>
                            </FormItem>
                        </div>
                      </div>
